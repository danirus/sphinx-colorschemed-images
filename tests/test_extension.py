...
from sphinx_colorschemed_images.exceptions import CSIExtensionException
...
def test_prj2_with_wrong_csi_image_path_pattern(test_app):
    ...
    with pytest.raises(CSIExtensionException) as excinfo:
        test_app(
            buildername="html",
            srcdir="sample_prj_2",
            confoverrides={
...
def test_prj4_has_wrong_image_uri(sphinx_logger, csi_logger, test_app):
    ...
    assert sphinx_logger.warning.call_count == 2
    assert sphinx_logger.warning.call_args_list[0].args == (
        __('image file not readable: %s'), 'img/image_none.png'
    )
    assert sphinx_logger.warning.call_args_list[1].args == (
        __('image file not readable: %s'), 'img/figure_none.png'
    )

    assert csi_logger.warning.called
    assert csi_logger.warning.call_count == 4
    assert csi_logger.warning.call_args_list[0].args == (
        __('image not found: %s'), 'img/image_none.en.light.png'
    )
    assert csi_logger.warning.call_args_list[1].args == (
        __('image not found: %s'), 'img/image_none.en.dark.png'
    )
    assert csi_logger.warning.call_args_list[2].args == (
        __('image not found: %s'), 'img/figure_none.en.light.png'
    )
    assert csi_logger.warning.call_args_list[3].args == (
        __('image not found: %s'), 'img/figure_none.en.dark.png'
    )
